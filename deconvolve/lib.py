...
def ess_gen_farina(
    ...
    sweep = np.sin(
        ((w1 * t_sweep) / np.log(R)) * (np.exp((t / t_sweep) * np.log(R)) - 1)
    )

    if fade_in > 0:
        sweep[0:fade_in] = sweep[0:fade_in] * np.sin(
            np.arange(0, fade_in) * (np.pi / (2 * fade_in))
        )
    ...
    if cut_zerocross:
        ...
        max_freq = f_start * np.exp(
            (t[sweep.shape[0] - num_samples_to_cut] / t_sweep) * np.log(R)
        )
...
def ess_parse_farina(
    ...
    if sweep.ndim > 1:
        raise Exception(
            "Input has more than one dimension. Please input"
            "a one dimensional vector containing the ESS."
        )
    duration = int(np.floor((t_sweep + t_idle) * fs))
    rir = np.array(sig.fftconvolve(sweep[:duration], inverse, mode="full"))
    if causality:
